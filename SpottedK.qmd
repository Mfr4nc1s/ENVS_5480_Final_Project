...
rast_data <- list.files("ta/DEMs", pattern = ".tif", full.names = TRUE)

# remove listed files that have either "_TEMP.tif" or "dem" in the name
rast_data <- grep("_TEMP.tif|dem", rast_data, value = TRUE, invert = TRUE)

# THIS IS TEMPORARY UNTIL THE LAYERS ARE FIXED: currently, the TPI, 
# openness_pos, and openness_neg files are not the correct files, so remove
# those as well:
rast_data <- grep("openness_|tpi.tif", rast_data, value = TRUE, invert = TRUE)

sk_raster <- rast(rast_data)
sk_raster
plot(sk_raster)

##load vector data & read in sf dataframe 
vect_data <-system.file("extdata", "LDB_SK.gpkg", package = "mlr3spatial") ##wrong?
vect_data <- c(table_join, vector, package="mlr3spatial") ##use
sk_vector <- st_read(vect_data, stringsAsFactors = TRUE)
sk_vector

##target is what is predicted
tsk_sk <- as_task_classif_st(sk_vector, target = "sk_pres")
tsk_sk
##created task
...
lrn_rf_tune <- lrn("classif.ranger", 
                   ...
                   mtry = to_tune(1, length(tsk_sk$feature_names??)),
                   importance = "impurity")
...
df_design <- expand.grid(
  ...
  mtry = 1:length(tsk_sk$feature_names))
...
rr_prob <- resample(tsk_sk, at_prob, cv_outer, store_models = TRUE)
